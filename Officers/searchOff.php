...
<body>
    ...
    ';
            ...
            $q_all_entries = "SELECT offName, offID, ID, contact, gender, weapon, role FROM officer";
            $result_all_entries = mysqli_query($conn, $q_all_entries);
            
            // Check if there are any entries
            if(mysqli_num_rows($result_all_entries) > 0) {
                echo '
                <table border="5" style="position:absolute; left:-50px;top:200px;">
                <tr>
                    <th>Officer Name</th>
                ...
                while ($row_all_entries = mysqli_fetch_array($result_all_entries)){
                    echo '
                    <tr>
                        <td>'.$row_all_entries['offName'].'</td>
                        <td>'.$row_all_entries['offID'].'</td>
                        <td>'.$row_all_entries['ID'].'</td>
                        <td>'.$row_all_entries['contact'].'</td>
                        <td>'.$row_all_entries['gender'].'</td>
                        <td>'.$row_all_entries['weapon'].'</td>
                        <td>'.$row_all_entries['role'].'</td>
                    </tr>';
                }

      ...
      <div class="container" style="height:780px;">
         <div class="finaldiv">
            ...
            <?php
                  ...
                  $total="";
                  $temp="";
                  $Ragging=$Robbery=$Kidnapping=$Rape=$Murder=$Fraud=$hitnRun=$suicide="";
                  ...
                  $hitnRun="SELECT COUNT('crime') from info where crime='hitnRun'";
                  $result=mysqli_query($conn,$hitnRun);
                  while ($row = mysqli_fetch_array($result)) {
                     $hitnRun=$row[0]/$total*100;
                     $row="";   
                  }  
                  $suicide="SELECT COUNT('crime') from info where crime='suicide'";
                  $result=mysqli_query($conn,$suicide);
                  while ($row = mysqli_fetch_array($result)) {
                     $suicide=$row[0]/$total*100;
                     $row="";
                  }   
                  echo'
                  <script>
                  anychart.onDocumentReady(function() {
                     var data = [
                        ...
                        {x: "Fraud", value:"'.$Fraud.'"},
                        {x: "hitnRun", value:"'.$hitnRun.'"},
                        {x: "suicide", value:"'.$suicide.'"},
                     ];
